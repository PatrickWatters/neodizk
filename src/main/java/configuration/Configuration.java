...
public class Configuration implements Serializable {
  ...
  /**
   * Prints the message and prints the total runtime of the code section. If beginLog() was not
   * invoked for this message, the resulting runtime is not returned.
   */
  public void endLog(final String message, final boolean indent) {
    if (verboseFlag) {
      ...
      if (totalTime == 0) {
        ...
        // Print message.
        System.out.println(
            "Ending   : " + message + " " + elapsedTimeInSeconds() + " - " + timeOutput);
      }
   ...
   * Note, if the specified context and header are not present in the records, then the necessary
   * records are created. If the last header record is not completed, then nothing is done. Observe
   * that 'null' is used to denote metadata status.
   */
  ...
  /**
   * Completes an existing runtime record for the specified context and header, setting boolean to
   * true, representing a completion of the time calculation for this record. Note, if the specified
   * context and header are not present in the records, then nothing is done. If the last header
   * record is already completed, then nothing is done.
   */
  ...
  public void writeRuntimeLog(final String context) {
    ...
    try {
      if (runtimeFiles.containsKey(context) && runtimeLogs.containsKey(context)) {
        ...
        final LinkedHashMap<String, Stack<Tuple2<Long, Boolean>>> headers =
            runtimeLogs.get(context);
  ...
  {
    Double val = (System.nanoTime() - startTime) / 1000000000.0;
  ...
  private void newRuntimeLog(final String context, final String... headers) {
    ...
    if (!runtimeLogs.containsKey(context)) {
      ...
      final LinkedHashMap<String, Stack<Tuple2<Long, Boolean>>> headersMap =
          runtimeLogs.get(context);
